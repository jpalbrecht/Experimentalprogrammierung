...
expparams3;         % experimental parameters *NOT* to be modified between subjects
...
try    % this is important: if there's an error, psychtoolbox will crash graciously
   ...
   save('filename3_sjind.mat','R');

   rew1 = load('filename1_sjind.mat','R');
   rew2 = load('filename2_sjind.mat','R');
   rew3 = load('filename3_sjind.mat','R');
   totRew = sum(rew1.R) + sum(rew2.R) + sum(rew3.R);
   payout = totRew;
