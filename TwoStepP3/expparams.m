...
if dotraining;	Z.Ntrials = 18;  nt0=19; 	    % number of dotraining trials DO NOT CHANGE unless you know what you do!
                                                % nt0 has to be > Ntrials.
else			   Z.Ntrials = 100; nt0=1;        % number of trials per BLOCK! & actual start trial
end
...
session  = '0';   % Which session (0, training, first 1-67trail, second 68-134 trail or third session 135-201 trail?)

% Settings for payout 
maxpayout=10; 		   % maximal payout in Euro for each block (Punishment, Reward)
                       % so one can win 10 Euro max in Punishment
                       % and 10 Euro max in Reward... together: 20Euro!
minpayout = 4;         % minimal payout in Euro for each block (Punishment, Reward)
punStartValue = 12;    % start value for punishment block
...
elseif strcmpi(devicetype,'lumitouch');	 % for lumitouch device
	...
	instrforward	= '3';		% index finger of non-dominant hand
	instrbackward	= '8';		% third finger of non-dominant hand
